#ifndef ABSMC_ENDO_3D_H
#define ABSMC_ENDO_3D_H
...
/// 3d IEL agent.
class Endo3D : public CellBase3D {
public:
    typedef AgentBase<3> base_t;
    typedef AgentRule<Endo3D> rule_t;
    typedef Point<3, double> point_t;

    enum CycleState { G0, G1, SG2M };

    Endo3D(point_t pos, point_t  pos0, double r)
        : CellBase3D(tEndo3D, pos, pos0, r),
		  rule(0), state(G0), age(0), clock(0.0), baFlag(true), ciFlag(false), noFlag(false),
          lengthG1(static_cast<int>(util::Random::getNextGauss(Endo3D::lengthG1Mean, Endo3D::lengthG1StdDev))) { }

	Endo3D(point_t pos, point_t  pos0, double r, double drugConc,  double wssOsi, double wssMax, double neoDist, double nitricConc,
			int age, double clock, int lengthG1,
			bool baFlag, bool ciFlag)
	: CellBase3D(tEndo3D, pos, pos0, r, drugConc, wssOsi, wssMax, neoDist, nitricConc), rule(0), state(G0),
	age(age), clock(clock), baFlag(baFlag), ciFlag(ciFlag),
    lengthG1(lengthG1) { }

    virtual Endo3D* clone() const { return new Endo3D(*this); }
    ...
    void setClock(double clock_) { clock = clock_; }
